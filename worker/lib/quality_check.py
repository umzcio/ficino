...
def check_extraction_quality(text: str) -> tuple[bool, str]:
    ...
    if len(stripped) < MIN_TEXT_LENGTH:
        logger.warn("quality_check_short", length=len(stripped))
    ...
    if artifact_count > 10:
        logger.warn("quality_check_artifacts", count=artifact_count)
    ...
    if density > MAX_SYMBOL_DENSITY:
        logger.warn("quality_check_symbols", density=round(density, 3))
    ...
    if words:
        ...
        if avg_word_len < MIN_AVG_WORD_LENGTH:
            logger.warn("quality_check_word_length", avg=round(avg_word_len, 2))
    ...
    if trigram_ratio < 0.05:
        logger.warn("quality_check_repetitive", ratio=round(trigram_ratio, 3))
